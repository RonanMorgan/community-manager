...
class TestSyncLogic(unittest.TestCase):
    ...
    def test_library_sync_single_group_user_added_successfully(self):
        ...
        self.mock_mm_client_instance.get_channel_by_name.return_value = {"id": "mm_chan_id1", "display_name": "Dev Team Sync"}
        ...
        self.assertEqual(users_added, 1)
    ...
    def test_library_sync_single_group_mm_channel_not_found(self):
        ...
        self.assertEqual(users_added, 0)
    ...
    def test_library_sync_single_group_client_missing(self):
        users_added = sync_single_authentik_group_with_mattermost(None, self.mock_mm_client_instance, self.test_mm_team_id, {}, {})
        self.assertIsNone(users_added)
    ...
    def test_library_orchestrate_sync_success(self, mock_get_groups_map, mock_sync_single):
        ...
        mock_sync_single.return_value = 1 # Each call to sync_single adds 1 user

        result = orchestrate_authentik_mattermost_sync(mock_auth_client, mock_mm_client, mock_team_id)

        self.assertTrue(result)
        mock_get_groups_map.assert_called_once_with(mock_auth_client)
        self.assertEqual(mock_sync_single.call_count, 2)
        mock_sync_single.assert_any_call(mock_auth_client, mock_mm_client, mock_team_id, mock_groups_list[0], mock_email_pk_map)
        mock_sync_single.assert_any_call(mock_auth_client, mock_mm_client, mock_team_id, mock_groups_list[1], mock_email_pk_map)
    ...
    def test_library_orchestrate_sync_no_groups_found(self, mock_get_groups_map):
        ...
        result = orchestrate_authentik_mattermost_sync(mock_auth_client, mock_mm_client, mock_team_id)
        self.assertTrue(result) # Still true as operation completed, just nothing to do
        mock_get_groups_map.assert_called_once_with(mock_auth_client)

    def test_library_orchestrate_sync_clients_missing(self):
        self.assertFalse(orchestrate_authentik_mattermost_sync(None, MagicMock(spec=MattermostClient), "team_id"))
        self.assertFalse(orchestrate_authentik_mattermost_sync(MagicMock(spec=AuthentikClient), None, "team_id"))
        self.assertFalse(orchestrate_authentik_mattermost_sync(MagicMock(spec=AuthentikClient), MagicMock(spec=MattermostClient), None))
    ...
    def test_script_main_sync_logic_orchestration(self, mock_orchestrate_lib, mock_script_init_clients, mock_script_config):
    ...
    @patch("scripts.sync_mm_authentik_groups.initialize_clients")
    @patch("scripts.sync_mm_authentik_groups.orchestrate_authentik_mattermost_sync")
    def test_script_main_sync_logic_init_auth_fails(self, mock_orchestrate_lib, mock_script_init_clients, mock_script_config):
        mock_script_config.MATTERMOST_TEAM_ID = "script_team_id"
    ...
    @patch("scripts.sync_mm_authentik_groups.initialize_clients")
    @patch("scripts.sync_mm_authentik_groups.orchestrate_authentik_mattermost_sync")
    def test_script_main_sync_logic_init_mm_fails(self, mock_orchestrate_lib, mock_script_init_clients, mock_script_config):
        mock_script_config.MATTERMOST_TEAM_ID = "script_team_id"
    ...
    @patch("scripts.sync_mm_authentik_groups.initialize_clients")
    @patch("scripts.sync_mm_authentik_groups.orchestrate_authentik_mattermost_sync")
    def test_script_main_sync_logic_no_team_id(self, mock_orchestrate_lib, mock_script_init_clients, mock_script_config):
