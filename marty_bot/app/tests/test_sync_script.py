...
class TestSyncLogic(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_brevo_client_instance = MagicMock(
            spec=BrevoClient
        )  # Added Brevo mock
    ...
    def test_script_initialize_clients_success(
        self, mock_script_config, MockScriptAuthClient, MockScriptMMClient
    ):
        ...
        mock_script_config.OUTLINE_URL = (
            "http://outline.example.com"  # Assume outline is configured
        )
        mock_script_config.OUTLINE_TOKEN = "outline_token"
        mock_script_config.BREVO_API_URL = (
            "http://brevo.example.com"  # Assume brevo is configured
        )
        ...
        with patch(
            "scripts.sync_mm_authentik_groups.OutlineClient"
        ) as MockScriptOutlineClient, patch(
            ...
            MockScriptAuthClient.assert_called_once_with(
                "http://auth.example.com", "auth_token"
            )
            MockScriptMMClient.assert_called_once_with(
                "http://mm.example.com", "mm_bot_token", "mm_team_id"
            )
            MockScriptOutlineClient.assert_called_once_with(
                "http://outline.example.com", "outline_token"
            )
            MockScriptBrevoClient.assert_called_once_with(
                "http://brevo.example.com", "brevo_key"
            )
            ...
            self.assertEqual(
                vw_client, mock_vaultwarden_instance
            )  # Added Vaultwarden check
            MockScriptVWClient.assert_called_once()  # Ensure VW Client was called

    @patch("scripts.sync_mm_authentik_groups.AuthentikClient")
    @patch("scripts.sync_mm_authentik_groups.config")
    def test_script_initialize_clients_auth_missing_config(
        self, mock_script_config, MockScriptAuthClient
    ):
        ...
        mock_script_config.VAULTWARDEN_ORGANIZATION_ID = (
            "vw_org"  # Ensure all config vars for other clients
        )
    ...
    @patch("scripts.sync_mm_authentik_groups.MattermostClient")
    @patch("scripts.sync_mm_authentik_groups.config")
    def test_script_initialize_clients_mm_missing_config(
        self, mock_script_config, MockScriptMMClient
    ):
    ...
    def test_library_get_all_authentik_groups_and_user_map(self):
        ...
        groups, email_map = get_all_authentik_groups_and_user_map(
    ...
    async def test_library_orchestrate_sync_success_all_clients(
        ...
        mock_lib_config.PERMISSIONS_MATRIX = {
            ...
            "ANTENNE": {
                "standard": {"authentik_group_name_pattern": "antenne_{base_name}"}
            },
        }
        ...
        success, detailed_results = (
            await orchestrate_group_synchronization(  # Added await
    ...
    async def test_library_orchestrate_sync_success_outline_client_none(
        ...
        mock_groups_list = [
            {"name": "projet_gamma", "pk": "g_gamma", "users": [], "users_obj": []}
        ]
        ...
        mock_sync_entity_permissions.return_value = [
            {"service": "AUTHENTIK_ONLY", "status": "SUCCESS"}
        ]
    ...
    @patch("libraries.group_sync_services.config")
    @async_test  # Added decorator
    async def test_library_orchestrate_sync_no_groups_found(
        self, mock_lib_config, mock_get_groups_map
    ):
        mock_auth_client = MagicMock(spec=AuthentikClient)
        mock_mm_client = MagicMock(spec=MattermostClient)
        mock_outline_client = MagicMock(spec=OutlineClient)
...
class TestVaultwardenSync(unittest.TestCase):
    ...
    @patch(
        "libraries.group_sync_services._map_vaultwarden_collection_to_entity_and_base_name"
    )
    def test_sync_vaultwarden_removes_user(
        ...
        self.assertEqual(
            results[0]["action"], "USER_REMOVED_FROM_VAULTWARDEN_COLLECTION"
        )
