...
class TestGroupSyncServices(unittest.TestCase):
    ...
    def test_map_auth_group_to_entity_and_base_name(self):
        ...
        self.assertEqual(_map_auth_group_to_entity_and_base_name("projet_MonProjet_admin", matrix), ("PROJET", "MonProjet"))
        ...
        self.assertEqual(_map_auth_group_to_entity_and_base_name("projet__admin", matrix), ("PROJET", "")) # Test expects "" now
        self.assertEqual(_map_auth_group_to_entity_and_base_name("antenne_MaRegion_standard", matrix), ("ANTENNE", "MaRegion"))
    ...
    def test_map_mm_channel_to_entity_and_base_name(self):
        ...
        self.assertEqual(_map_mm_channel_to_entity_and_base_name("projet-alpha", "Projet Alpha", matrix), ("PROJET", "Alpha"))
        self.assertEqual(_map_mm_channel_to_entity_and_base_name("projet-alpha-admin", "Projet Alpha Admin", matrix), ("PROJET", "Alpha"))
        self.assertEqual(_map_mm_channel_to_entity_and_base_name("antenne-maregion-standard", "Antenne MaRegion Standard", matrix), ("ANTENNE", "MaRegion"))
        self.assertIsNone(_map_mm_channel_to_entity_and_base_name("unknown-channel", "Unknown Channel Format", matrix)[0])
        matrix_slug_friendly = {
             "PROJET": {"standard": {"mattermost_channel_name_pattern": "projet-{base_name}"}}
        }
        self.assertEqual(_map_mm_channel_to_entity_and_base_name("projet-my-cool-project", "DIFFERENT DISPLAY NAME", matrix_slug_friendly), ("PROJET", "my-cool-project"))
    ...
    def test_sync_single_group_user_exclusion(self, mock_config_module_in_service):
        ...
        mm_users_std_channel = [{"username": "user1", "email": "user1@example.com", "id": "mm_user_id_1"},{"username": "excluded_user", "email": "excludeduser@example.com", "id": "mm_user_id_excluded"},]
        mm_users_adm_channel = [{"username": "user1", "email": "user1@example.com", "id": "mm_user_id_1"},{"username": "user2", "email": "user2@example.com", "id": "mm_user_id_2"},{"username": "marty", "email": "marty@example.com", "id": "mm_user_id_marty"},]
...
"""
    ...
    def test_sync_single_group_authentik_user_removed_if_not_in_mm(self, mock_config_module_in_service):
        ...
        auth_user_obj_to_remove = {"pk": auth_user_pk_to_remove,"email": "removeme@example.com","username": "removeme_user",}
        ...
        mock_entity_config = {
            "standard": {"authentik_group_name_pattern": current_auth_group_name,"mattermost_channel_name_pattern": current_auth_group_name,}
        }
    ...
    def test_sync_single_group_authentik_excluded_user_not_removed_if_not_in_mm(self, mock_config_module_in_service):
        ...
        auth_user_obj_excluded = {"pk": auth_user_pk_excluded,"email": "excluded@example.com","username": excluded_auth_username,}
        ...
        for base_name_from_case, mm_channel_type_being_processed, expected_permission in test_cases:
            ...
            with self.subTest(subTest_name):
                ...
                if outline_result:
                    ...
                    expected_action_val = (f"USER_ADDED_TO_OUTLINE_COLLECTION_WITH_{expected_permission.upper()}_ACCESS{dm_part}")
    ...
    def test_orchestrate_sync_fetch_remote_false_discover_via_mm_no_deletions(
        ...
        mock_lib_config.PERMISSIONS_MATRIX = { # Matrix still needed for _map_mm_channel_to_entity_and_base_name if it were called
        ...
        success, detailed_results = orchestrate_group_synchronization(
            self.mock_authentik_client, self.mock_mattermost_client, self.mock_outline_client, mock_team_id,
            perform_deletions=False, fetch_remote_members=False
        )
    ...
    def test_orchestrate_sync_fetch_remote_true_discover_via_auth_with_deletions(
        ...
        success, detailed_results = orchestrate_group_synchronization(
            self.mock_authentik_client, self.mock_mattermost_client, self.mock_outline_client, mock_team_id,
            perform_deletions=True, fetch_remote_members=True
        )
        ...
        mock_sync_entity_permissions_call.assert_any_call(
            self.mock_authentik_client, self.mock_mattermost_client, self.mock_outline_client, mock_team_id,
            "Gamma", "PROJET", mock_lib_config.PERMISSIONS_MATRIX["PROJET"],
            expected_all_auth_groups_by_name, mock_email_pk_map, True
        )
        mock_sync_entity_permissions_call.assert_any_call(
            self.mock_authentik_client, self.mock_mattermost_client, self.mock_outline_client, mock_team_id,
            "Delta", "ANTENNE", mock_lib_config.PERMISSIONS_MATRIX["ANTENNE"],
            expected_all_auth_groups_by_name, mock_email_pk_map, True
        )
