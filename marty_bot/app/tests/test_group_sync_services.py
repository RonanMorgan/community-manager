...
from libraries.group_sync_services import (
    ...
    _extract_base_name,
    # _sync_single_vaultwarden_collection, # Not testing this private method directly here yet
)
...
from clients.vaultwarden_client import VaultwardenClient # Ensure this is imported
...
class TestGroupSyncServices(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_vaultwarden_client = MagicMock(spec=VaultwardenClient) # Initialized
        ...
        self.mock_vaultwarden_client.create_collection.return_value = {"id": "default_vw_id", "name": "DefaultVWCollection"}
    ...
    def test_sync_single_group_user_exclusion(self, mock_config_module_in_service):
        ...
        mock_entity_config = {
            ...
            "vaultwarden": { # Added Vaultwarden config for this test
                 "collection_name_pattern": "vw_projet_{base_name}"
            }
        }
        std_auth_group_name = mock_entity_config["standard"]["authentik_group_name_pattern"].format(base_name=base_name)
        std_mm_channel_name = mock_entity_config["standard"]["mattermost_channel_name_pattern"].format(base_name=base_name)
        adm_auth_group_name = mock_entity_config["admin"]["authentik_group_name_pattern"].format(base_name=base_name)
        adm_mm_channel_name = mock_entity_config["admin"]["mattermost_channel_name_pattern"].format(base_name=base_name)
        outline_coll_name = mock_entity_config["outline"]["collection_name_pattern"].format(base_name=base_name)
        vw_coll_name = mock_entity_config["vaultwarden"]["collection_name_pattern"].format(base_name=base_name)
        ...
        def get_channel_by_name_side_effect(team_id, channel_name_slug):
            if channel_name_slug == slugify(std_mm_channel_name): return std_mm_channel_obj
            if channel_name_slug == slugify(adm_mm_channel_name): return adm_mm_channel_obj
            return None
        ...
        def get_users_in_channel_side_effect(channel_id):
            if channel_id == std_mm_channel_obj["id"]: return mm_users_std_channel
            if channel_id == adm_mm_channel_obj["id"]: return mm_users_adm_channel
            return []
        ...
        self.mock_outline_client.get_collection_details.return_value = {"id": expected_outline_coll_id, "name": outline_coll_name}
        ...
        self.mock_outline_client.get_user_by_email.side_effect = lambda email: ({"id": f"outlineid_{email.split('@')[0]}"} if email else None)
        self.mock_mattermost_client.send_dm.return_value = True
        self.mock_vaultwarden_client.create_collection.return_value = {"id": "vw_id_1", "name": vw_coll_name}
        ...
        results = sync_entity_permissions(
            ...
            vaultwarden_client=self.mock_vaultwarden_client, # Pass client
        ...
        self.mock_outline_client.add_user_to_collection.assert_any_call(expected_outline_coll_id, outline_user1_id, permission="read_write")
        self.mock_outline_client.add_user_to_collection.assert_any_call(expected_outline_coll_id, outline_user2_id, permission="read_write")
        ...
        for call in calls: self.assertEqual(call.args[0], expected_outline_coll_id)

        self.mock_vaultwarden_client.create_collection.assert_called_once_with(vw_coll_name) # Assert VW call

        successful_actions = [r for r in results if r.get("status") == "SUCCESS"]
        self.assertEqual(len(successful_actions), 6) # Auth (3) + Outline (2) + VW (1)
        for r in results:
            if r.get("mm_username") in {"excluded_user", "marty"}:
                self.assertNotEqual(r.get("status"), "SUCCESS", f"Excluded user {r.get('mm_username')} had a SUCCESS action.")
                self.assertNotEqual(r.get("status"), "FAILURE", f"Excluded user {r.get('mm_username')} had a FAILURE action.")
    ...
    def test_config_loading_file_not_found(self, mock_exists, mock_open_file, mock_getenv, mock_find_dotenv):
        def getenv_side_effect(key, default=None):
            if key == "EXCLUDED_USERS_FILE_PATH": return "dummy_path/non_existent_excluded.txt"
            if key == "PERMISSIONS_MATRIX_FILE_PATH": return "dummy_path/non_existent_matrix.yml"
    ...
    def test_config_loading_empty_file(self, mock_exists, mock_open_file, mock_getenv, mock_find_dotenv):
        ...
        def getenv_side_effect(key, default=None):
            if key == "EXCLUDED_USERS_FILE_PATH": return dummy_excluded_path
            if key == "PERMISSIONS_MATRIX_FILE_PATH": return dummy_matrix_path
            return os.environ.get(key, default)
        mock_getenv.side_effect = getenv_side_effect
...
"""
    ...
    def test_sync_single_group_outline_dm_on_new_add(self, mock_config_module_in_service):
        ...
        mock_config_module_in_service.OUTLINE_URL = "http://fake-outline.com"
        base_name = "DMTestProject"; entity_key = "PROJET"
        mock_entity_config = {
            "standard": {"authentik_group_name_pattern": "projet_{base_name}", "mattermost_channel_name_pattern": "projet_{base_name}"},
        ...
        self.mock_mattermost_client.get_channel_by_name.side_effect = lambda _, slug: (std_mm_channel_obj if slug == slugify(std_mm_channel_name) else None)
        self.mock_mattermost_client.get_users_in_channel.return_value = [mm_user_for_dm]
        self.mock_authentik_client.add_user_to_group.return_value = True
        self.mock_outline_client.get_user_by_email.return_value = {"id": "outline_user_id_dm", "email": "dmuser@example.com"}
        ...
        self.mock_outline_client.get_collection_members.return_value = []
        self.mock_outline_client.add_user_to_collection.return_value = True
        self.mock_outline_client.get_collection_details.return_value = {"id": expected_collection_id, "name": outline_coll_name}
        self.mock_mattermost_client.send_dm.return_value = True
        results = sync_entity_permissions(
            authentik_client=self.mock_authentik_client, mattermost_client=self.mock_mattermost_client,
            outline_client=self.mock_outline_client, brevo_client=self.mock_brevo_client,
            vaultwarden_client=self.mock_vaultwarden_client, # Pass client
            mm_team_id=self.mm_team_id, entity_key=entity_key, base_name=base_name, entity_config=mock_entity_config,
            all_authentik_groups_by_name=all_authentik_groups_by_name_fixture,
            email_to_authentik_user_pk_map=email_map_for_dm, perform_deletions=True,
        )
        # Adjusted count for potentially new VW success
        self.assertEqual(len([r for r in results if r["status"] == "SUCCESS" and r["service"] != "VAULTWARDEN"]), 3)
        ...
        expected_url = f"{mock_config_module_in_service.OUTLINE_URL}/collection/{collection_slug}-{expected_collection_id}"
        self.assertIn(expected_url, call_args[1])
        self.assertIn(outline_coll_name, call_args[1])
        self.mock_outline_client.create_group.assert_called_once_with(outline_coll_name)
        self.mock_outline_client.add_user_to_collection.assert_called_once_with(expected_collection_id, "outline_user_id_dm", permission="read")

    @patch("libraries.group_sync_services.config")
    def test_sync_single_group_outline_dm_fails(self, mock_config_module_in_service):
        mock_config_module_in_service.EXCLUDED_USERS = set()
        mock_config_module_in_service.OUTLINE_URL = "http://fake-outline.com"
        base_name = "DMFailProject"; entity_key = "PROJET"
        mock_entity_config = {
            "standard": {"authentik_group_name_pattern": "projet_{base_name}", "mattermost_channel_name_pattern": "projet_{base_name}"},
            "outline": {"collection_name_pattern": "projet_{base_name}", "default_access": "read", "admin_access": "read_write"},
            "brevo": {"list_name_pattern": "brevo_projet_{base_name}"},
        ...
        self.mock_mattermost_client.get_users_in_channel.return_value = [mm_user_for_dm]
        self.mock_authentik_client.add_user_to_group.return_value = True
        self.mock_outline_client.get_user_by_email.return_value = {"id": "outline_user_id_dm_fail", "email": "dmuserfail@example.com"}
        expected_collection_id = f"new_outline_id_for_{slugify(outline_coll_name)}"
        self.mock_outline_client.create_group.return_value = {"id": expected_collection_id, "name": outline_coll_name}
        self.mock_outline_client.get_collection_members.return_value = []
        self.mock_outline_client.add_user_to_collection.return_value = True
    ...
    def test_sync_single_group_outline_user_already_member_no_dm(self, mock_config_module_in_service):
        mock_config_module_in_service.EXCLUDED_USERS = set()
        mock_config_module_in_service.OUTLINE_URL = "http://fake-outline.com"
        base_name = "AlreadyMemberProject"; entity_key = "PROJET"
        mock_entity_config = {
            "standard": {"authentik_group_name_pattern": "projet_{base_name}", "mattermost_channel_name_pattern": "projet_{base_name}"},
            "outline": {"collection_name_pattern": "projet_{base_name}", "default_access": "read", "admin_access": "read_write"},
        }
        ...
        results = sync_entity_permissions(
            ...
            mm_team_id=self.mm_team_id, entity_key=entity_key, base_name=base_name, entity_config=mock_entity_config,
            all_authentik_groups_by_name=all_authentik_groups_by_name_fixture,
            email_to_authentik_user_pk_map=email_map_already, perform_deletions=True,
        )
        outline_result = next(r for r in results if r["service"] == "OUTLINE" and r["status"] == "SUCCESS")
        self.assertEqual(outline_result["action"], "USER_ALREADY_IN_OUTLINE_COLLECTION_PERMISSION_ENSURED")
        self.mock_outline_client.add_user_to_collection.assert_called_once_with(expected_collection_id, outline_user_data["id"], permission="read")
    ...
    def test_sync_single_group_authentik_user_removed_if_not_in_mm(self, mock_config_module_in_service):
        mock_config_module_in_service.EXCLUDED_USERS = set()
        auth_user_pk_to_remove = "auth_pk_to_remove"; auth_user_obj_to_remove = {"pk": auth_user_pk_to_remove, "email": "removeme@example.com", "username": "removeme_user"}
        auth_user_pk_to_keep = "auth_pk_to_keep"; auth_user_obj_to_keep = {"pk": auth_user_pk_to_keep, "email": "keepme@example.com", "username": "keepme_user"}
        mm_user_to_keep = {"username": "keepme_user", "email": "keepme@example.com", "id": "mm_id_keep"}
        current_auth_group_name = "AuthGroupForRemovalTest"; current_auth_group_pk = "auth_group_pk_removal"
        authentik_group_with_users = {"name": current_auth_group_name, "pk": current_auth_group_pk, "users": [auth_user_pk_to_remove, auth_user_pk_to_keep], "users_obj": [auth_user_obj_to_remove, auth_user_obj_to_keep]}
        ...
        self.mock_authentik_client.add_user_to_group.return_value = True
        mock_entity_config = {"standard": {"authentik_group_name_pattern": current_auth_group_name, "mattermost_channel_name_pattern": current_auth_group_name}}
        all_auth_groups_fixture = {current_auth_group_name: authentik_group_with_users}
        results = sync_entity_permissions(
            authentik_client=self.mock_authentik_client, mattermost_client=self.mock_mattermost_client, outline_client=None,
            brevo_client=self.mock_brevo_client, vaultwarden_client=self.mock_vaultwarden_client, # Pass client
            mm_team_id=self.mm_team_id, entity_key="PROJET", base_name="", entity_config=mock_entity_config,
            all_authentik_groups_by_name=all_auth_groups_fixture, email_to_authentik_user_pk_map=email_to_pk_map, perform_deletions=True,
        )
        self.mock_authentik_client.remove_user_from_group.assert_called_once_with(current_auth_group_pk, auth_user_pk_to_remove)
        self.mock_authentik_client.add_user_to_group.assert_not_called()
        removal_action_found = any(r["service"] == "AUTHENTIK" and r["action"] == "USER_REMOVED_FROM_AUTHENTIK_GROUP" and r["mm_username"] == "removeme_user" for r in results)
        self.assertTrue(removal_action_found, "USER_REMOVED_FROM_AUTHENTIK_GROUP action not found for 'removeme_user'")
        kept_action_found = any(r["service"] == "AUTHENTIK" and r["action"] == "USER_ALREADY_IN_AUTHENTIK_GROUP" and r["mm_username"] == "keepme_user" for r in results)
    ...
    def test_sync_single_group_authentik_excluded_user_not_removed_if_not_in_mm(self, mock_config_module_in_service):
        ...
        auth_user_pk_excluded = "auth_pk_excluded_removal"; auth_user_obj_excluded = {"pk": auth_user_pk_excluded, "email": "excluded@example.com", "username": excluded_auth_username}
        current_auth_group_name = "AuthGroupForExcludedRemovalTest"; current_auth_group_pk = "auth_group_pk_excl_removal"
        authentik_group_with_excluded_user = {"name": current_auth_group_name, "pk": current_auth_group_pk, "users": [auth_user_pk_excluded], "users_obj": [auth_user_obj_excluded]}
        ...
        self.assertFalse(action_for_excluded_user_found, "No action should be logged for excluded user not in MM channel during removal phase.")

    @patch("libraries.group_sync_services.config")
    def test_sync_single_group_outline_user_removed_if_not_in_mm(self, mock_config_module_in_service):
        mock_config_module_in_service.EXCLUDED_USERS = set(); mock_config_module_in_service.OUTLINE_URL = "http://fake-outline.com"
        outline_user_id_to_remove = "outline_id_remove"; outline_user_id_to_keep = "outline_id_keep"
        mm_user_to_keep_outline = {"username": "keepme_outline", "email": "keepme.outline@example.com", "id": "mm_id_keep_outline"}
        base_name_for_test = "OutlineRemovalTest"; entity_key_for_test = "PROJET"
        std_auth_group_name = f"projet_{base_name_for_test}"; auth_group = {"name": std_auth_group_name, "pk": "auth_pk_outline_remove", "users": [], "users_obj": []}
        all_auth_groups_fixture = {std_auth_group_name: auth_group}
        email_to_pk_map = {mm_user_to_keep_outline["email"]: "auth_pk_keep_outline"}
        std_mm_channel_name = f"projet_{base_name_for_test}"; std_mm_channel_obj = {"id": "std_mm_chan_outline_remove", "display_name": std_mm_channel_name}
        self.mock_mattermost_client.get_channel_by_name.side_effect = lambda _, slug: (std_mm_channel_obj if slug == slugify(std_mm_channel_name) else None)
        self.mock_mattermost_client.get_users_in_channel.return_value = [mm_user_to_keep_outline]
        outline_coll_name = f"projet_{base_name_for_test}"; expected_collection_id = f"new_outline_id_for_{slugify(outline_coll_name)}"
        self.mock_outline_client.create_group.return_value = {"id": expected_collection_id, "name": outline_coll_name}
        self.mock_outline_client.get_collection_members.return_value = [outline_user_id_to_remove, outline_user_id_to_keep]
        def mock_get_user_by_email_side_effect(email):
            if email == mm_user_to_keep_outline["email"]: return {"id": outline_user_id_to_keep, "email": email}
            return None
        ...
        self.mock_outline_client.add_user_to_collection.return_value = True
        mock_entity_config = {
            "standard": {"authentik_group_name_pattern": std_auth_group_name, "mattermost_channel_name_pattern": std_mm_channel_name},
            "outline": {"collection_name_pattern": outline_coll_name, "default_access": "read", "admin_access": "read_write"},
        }
        results = sync_entity_permissions(
            authentik_client=self.mock_authentik_client, mattermost_client=self.mock_mattermost_client,
            outline_client=self.mock_outline_client, brevo_client=self.mock_brevo_client,
            vaultwarden_client=self.mock_vaultwarden_client, # Pass client
            mm_team_id=self.mm_team_id, entity_key=entity_key_for_test, base_name=base_name_for_test, entity_config=mock_entity_config,
            all_authentik_groups_by_name=all_auth_groups_fixture, email_to_authentik_user_pk_map=email_to_pk_map, perform_deletions=True,
        )
        self.mock_outline_client.remove_user_from_collection.assert_called_once_with(expected_collection_id, outline_user_id_to_remove)
        self.mock_outline_client.add_user_to_collection.assert_called_once_with(expected_collection_id, outline_user_id_to_keep, permission="read")
        ...
        if removal_action: self.assertEqual(removal_action["mm_username"], f"OutlineUserName_{outline_user_id_to_remove}")
        kept_action = next((r for r in results if r.get("action") == "USER_ALREADY_IN_OUTLINE_COLLECTION_PERMISSION_ENSURED"), None)
        self.assertIsNotNone(kept_action, "USER_ALREADY_IN_OUTLINE_COLLECTION_PERMISSION_ENSURED not found for kept user")
        if kept_action: self.assertEqual(kept_action["mm_username"], "keepme_outline")
        self.mock_outline_client.create_group.assert_called_once_with(outline_coll_name)
    ...
    def test_sync_single_group_outline_excluded_user_not_removed(self, mock_config_module_in_service):
        ...
        mock_config_module_in_service.EXCLUDED_USERS = {excluded_mm_username}; mock_config_module_in_service.OUTLINE_URL = "http://fake-outline.com"
        outline_id_excluded = "outline_id_excl"; base_name_for_test = "OutlineExcludedTest"; entity_key_for_test = "PROJET"
        std_auth_group_name = f"projet_{base_name_for_test}"; auth_group = {"name": std_auth_group_name, "pk": "auth_pk_excl_outline", "users": [], "users_obj": []}
        all_auth_groups_fixture = {std_auth_group_name: auth_group}
        email_to_pk_map = {"excluded.outline@example.com": "auth_pk_excl_for_outline_test"}
        std_mm_channel_name = f"projet_{base_name_for_test}"; std_mm_channel_obj = {"id": "std_mm_chan_excl_outline", "display_name": std_mm_channel_name}
        mm_user_excluded_in_channel = {"username": excluded_mm_username, "email": "excluded.outline@example.com", "id": "mm_id_excl"}
        self.mock_mattermost_client.get_channel_by_name.side_effect = lambda _, slug: (std_mm_channel_obj if slug == slugify(std_mm_channel_name) else None)
        ...
        self.mock_outline_client.get_user_by_email.return_value = {"id": outline_id_excluded, "email": "excluded.outline@example.com"}
        mock_entity_config = {
            "standard": {"authentik_group_name_pattern": std_auth_group_name, "mattermost_channel_name_pattern": std_mm_channel_name},
            "outline": {"collection_name_pattern": outline_coll_name, "default_access": "read"},
        }
        results = sync_entity_permissions(
            authentik_client=self.mock_authentik_client, mattermost_client=self.mock_mattermost_client,
            outline_client=self.mock_outline_client, brevo_client=self.mock_brevo_client,
        ...
        action_for_excluded_user_in_outline = any(r["service"] == "OUTLINE" and r.get("mm_username") == excluded_mm_username for r in results)
        self.assertFalse(action_for_excluded_user_in_outline, "No Outline action should be logged for an excluded user.")
        self.mock_outline_client.create_group.assert_called_once_with(outline_coll_name)
    ...
    def test_sync_single_group_outline_permissions(self, mock_config_module_in_service):
        mock_config_module_in_service.EXCLUDED_USERS = set(); mock_config_module_in_service.OUTLINE_URL = "http://fake-outline.com"
        test_cases = [
            ("projet_test", "O", "read"), ("projet_test", "P", "read_write"),
            ("antenne_test", "O", "read"), ("antenne_test", "P", "read_write"),
            ("pole_test", "O", "read"), ("pole_test", "P", "read_write"),
        ]
        ...
        for base_name_from_case, mm_channel_type_being_processed, expected_permission in test_cases:
            ...
            if "antenne" in base_name_from_case.lower(): entity_key_for_test = "ANTENNE"
            elif "pole" in base_name_from_case.lower(): entity_key_for_test = "POLES"
            subTest_name = f"base:{base_name_from_case}, chan_type_proc:{mm_channel_type_being_processed}, entity:{entity_key_for_test}"
            with self.subTest(subTest_name):
                self.mock_authentik_client.reset_mock(); self.mock_mattermost_client.reset_mock(); self.mock_outline_client.reset_mock()
                def default_outline_create_group_side_effect(name): return {"name": name, "id": f"new_outline_id_for_{slugify(name)}"}
                self.mock_outline_client.create_group.side_effect = default_outline_create_group_side_effect
                self.mock_outline_client.get_user_by_email.return_value = outline_user_data
                mock_entity_config = {
                    "standard": {"authentik_group_name_pattern": f"{entity_key_for_test.lower()}_{{base_name}}", "mattermost_channel_name_pattern": f"{entity_key_for_test.lower()}_{{base_name}}"},
                    "admin": {"authentik_group_name_pattern": f"{entity_key_for_test.lower()}_{{base_name}} Admin", "mattermost_channel_name_pattern": f"{entity_key_for_test.lower()}_{{base_name}} Admin"},
                    "outline": {"collection_name_pattern": f"{entity_key_for_test.lower()}_{{base_name}}", "default_access": "read", "admin_access": "read_write"},
                }
                std_auth_group_name = mock_entity_config["standard"]["authentik_group_name_pattern"].format(base_name=base_name_from_case)
                std_mm_channel_name = mock_entity_config["standard"]["mattermost_channel_name_pattern"].format(base_name=base_name_from_case)
                adm_auth_group_name = mock_entity_config["admin"]["authentik_group_name_pattern"].format(base_name=base_name_from_case)
                adm_mm_channel_name = mock_entity_config["admin"]["mattermost_channel_name_pattern"].format(base_name=base_name_from_case)
                outline_coll_name = mock_entity_config["outline"]["collection_name_pattern"].format(base_name=base_name_from_case)
                expected_outline_coll_id = f"new_outline_id_for_{slugify(outline_coll_name)}"
                std_auth_group_obj = {"name": std_auth_group_name, "pk": "std_auth_pk_perm_test", "users": [], "users_obj": []}
                adm_auth_group_obj = {"name": adm_auth_group_name, "pk": "adm_auth_pk_perm_test", "users": [], "users_obj": []}
                all_authentik_groups_by_name_fixture = {std_auth_group_name: std_auth_group_obj, adm_auth_group_name: adm_auth_group_obj}
                ...
                def get_channel_by_name_side_effect(team_id, channel_name_slug):
                    ...
                    return None
                self.mock_mattermost_client.get_channel_by_name.side_effect = get_channel_by_name_side_effect
                mm_users_for_std_channel = []; mm_users_for_adm_channel = []
                if mm_channel_type_being_processed == "O": mm_users_for_std_channel = [mm_user_fixture]
                elif mm_channel_type_being_processed == "P": mm_users_for_std_channel = [mm_user_fixture]; mm_users_for_adm_channel = [mm_user_fixture]
                def get_users_in_channel_side_effect(channel_id):
                    if channel_id == std_mm_channel_obj["id"]: return mm_users_for_std_channel
                    if channel_id == adm_mm_channel_obj["id"]: return mm_users_for_adm_channel
                    return []
                self.mock_mattermost_client.get_users_in_channel.side_effect = get_users_in_channel_side_effect
                self.mock_authentik_client.add_user_to_group.return_value = True
                self.mock_outline_client.get_collection_members.return_value = []
                ...
                results = sync_entity_permissions(
                    ...
                    outline_client=self.mock_outline_client, brevo_client=self.mock_brevo_client,
                    vaultwarden_client=self.mock_vaultwarden_client, # Pass client
                    mm_team_id=self.mm_team_id, entity_key=entity_key_for_test, base_name=base_name_from_case, entity_config=mock_entity_config,
                    all_authentik_groups_by_name=all_authentik_groups_by_name_fixture, email_to_authentik_user_pk_map=email_to_pk_map, perform_deletions=True,
                )
                self.mock_outline_client.create_group.assert_called_once_with(outline_coll_name)
                ...
                outline_result = next((r for r in results if r["service"] == "OUTLINE" and r["status"] == "SUCCESS"), None)
                self.assertIsNotNone(outline_result, f"No successful Outline result found for {subTest_name}")
                if outline_result:
                    dm_part = "_AND_DM_SENT"; expected_action_val = f"USER_ADDED_TO_OUTLINE_COLLECTION_WITH_{expected_permission.upper()}_ACCESS{dm_part}"
    ...
    @patch("libraries.group_sync_services.config")
    def test_orchestrate_sync_fetch_remote_false_discover_via_mm_no_deletions(self, mock_lib_config, mock_sync_entity_permissions_call):
        self.mock_authentik_client.reset_mock(); self.mock_mattermost_client.reset_mock(); self.mock_outline_client.reset_mock(); self.mock_vaultwarden_client.reset_mock()
        def create_auth_group_side_effect(name): return {"name": name, "pk": f"auth_pk_{slugify(name)}", "users": [], "users_obj": []}
        self.mock_authentik_client.get_group_by_name.return_value = None
        self.mock_authentik_client.create_group.side_effect = create_auth_group_side_effect
        def create_outline_coll_side_effect(name): return {"name": name, "id": f"outline_id_{slugify(name)}"}
        self.mock_outline_client.create_group.side_effect = create_outline_coll_side_effect
        mock_team_id = "team_upsert_mode"
        mock_email_pk_map_from_client = {"user.alpha@example.com": "auth_pk_alpha", "user.beta@example.com": "auth_pk_beta"}
        ...
        mock_lib_config.PERMISSIONS_MATRIX = {
            "PROJET": {"standard": {"mattermost_channel_name_pattern": "PROJET {base_name}", "authentik_group_name_pattern": "auth_projet_{base_name}"}},
            "ANTENNE": {"admin": {"mattermost_channel_name_pattern": "ANTENNE {base_name} Admin", "authentik_group_name_pattern": "auth_antenne_{base_name}_admin"}},
            "BREVO_TEST_ENTITY": {"standard": {"mattermost_channel_name_pattern": "brevo_test_{base_name}"}, "brevo": {"list_name_pattern": "brevo_list_{base_name}"}},
        }
        success, detailed_results = orchestrate_group_synchronization(
            self.mock_authentik_client, self.mock_mattermost_client, self.mock_outline_client, self.mock_brevo_client,
            self.mock_vaultwarden_client, # Pass client
            mock_team_id, perform_deletions=False, fetch_remote_members=False,
        )
        self.assertTrue(success); self.assertEqual(len(detailed_results), 0)
    ...
    @patch("libraries.group_sync_services.sync_entity_permissions")
    @patch("libraries.group_sync_services.config")
    def test_orchestrate_sync_fetch_remote_true_discover_via_auth_with_deletions(self, mock_lib_config, mock_sync_entity_permissions_call):
        self.mock_authentik_client.reset_mock(); self.mock_mattermost_client.reset_mock(); self.mock_outline_client.reset_mock(); self.mock_vaultwarden_client.reset_mock()
        def create_outline_coll_side_effect(name): return {"name": name, "id": f"outline_id_{slugify(name)}"}
        self.mock_outline_client.create_group.side_effect = create_outline_coll_side_effect
        mock_team_id = "team_sync_mode"
        auth_group_projet_gamma = {"name": "auth_projet_Gamma", "pk": "auth_g_gamma", "users": ["u1"], "users_obj": [{"pk": "u1", "email": "user.gamma@example.com", "username": "user.gamma"}]}
        auth_group_antenne_delta_admin = {"name": "auth_antenne_Delta_admin", "pk": "auth_g_delta_adm", "users": [], "users_obj": []}
        mock_all_auth_groups_list = [auth_group_projet_gamma, auth_group_antenne_delta_admin]
        self.mock_authentik_client.get_groups_with_users.return_value = (mock_all_auth_groups_list, {"some_email_from_groups_obj@example.com": "pk_temp"})
        ...
        success, detailed_results = orchestrate_group_synchronization(
            self.mock_authentik_client, self.mock_mattermost_client, self.mock_outline_client, self.mock_brevo_client,
            self.mock_vaultwarden_client, # Pass client
            mock_team_id, perform_deletions=True, fetch_remote_members=True,
        )
        self.assertTrue(success); self.assertEqual(len(detailed_results), 2)
        ...
        mock_sync_entity_permissions_call.assert_any_call(
            self.mock_authentik_client, self.mock_mattermost_client, self.mock_outline_client, self.mock_brevo_client,
            self.mock_vaultwarden_client, # Expect client
            mock_team_id, "Gamma", "PROJET", mock_lib_config.PERMISSIONS_MATRIX["PROJET"],
            expected_all_auth_groups_by_name, mock_email_pk_map_from_all_users, True,
        )
        mock_sync_entity_permissions_call.assert_any_call(
            self.mock_authentik_client, self.mock_mattermost_client, self.mock_outline_client, self.mock_brevo_client,
            self.mock_vaultwarden_client, # Expect client
            mock_team_id, "Delta", "ANTENNE", mock_lib_config.PERMISSIONS_MATRIX["ANTENNE"],
            expected_all_auth_groups_by_name, mock_email_pk_map_from_all_users, True,
        )
    ...
    def test_sync_brevo_list_creation_and_user_add(self, mock_lib_config_brevo):
        ...
        brevo_list_name = "TestBrevoList1"; mm_users = [{"username": "brevo_user1", "email": "brevo1@example.com"}, {"username": "brevo_user2", "email": "brevo2@example.com"}]
        ...
        self.mock_brevo_client.add_contact_to_list.return_value = True
        results = self.sync_single_brevo_list_helper(self.mock_brevo_client, brevo_list_name, mm_users, mm_channel_name_log, perform_deletions=False)
        ...
        self.mock_brevo_client.add_contact_to_list.assert_any_call(email="brevo1@example.com", list_id=expected_created_list_id)
        self.mock_brevo_client.add_contact_to_list.assert_any_call(email="brevo2@example.com", list_id=expected_created_list_id)
        self.assertEqual(len(results), 2)
        for res in results: self.assertEqual(res["status"], "SUCCESS"); self.assertEqual(res["action"], "USER_ENSURED_IN_BREVO_LIST"); self.assertEqual(res["service"], "BREVO")

    @patch("libraries.group_sync_services.config")
    def test_sync_brevo_list_user_removal(self, mock_lib_config_brevo):
        mock_lib_config_brevo.EXCLUDED_USERS = set()
        brevo_list_name = "TestBrevoListRemoval"; existing_list_obj = {"id": "brevo_list_id_456", "name": brevo_list_name}
        self.mock_brevo_client.get_list_by_name.return_value = existing_list_obj
        self.mock_brevo_client.create_list.assert_not_called()
        ...
        self.mock_brevo_client.remove_contact_from_list.return_value = True
        self.mock_brevo_client.add_contact_to_list.return_value = True
        results = self.sync_single_brevo_list_helper(self.mock_brevo_client, brevo_list_name, mm_users_in_channel, "MMChannelForBrevoRemoval", perform_deletions=True)
        self.mock_brevo_client.get_contacts_from_list.assert_called_once_with(existing_list_obj["id"], limit=50, offset=0)
        self.mock_brevo_client.remove_contact_from_list.assert_called_once_with(email="remove@example.com", list_id=existing_list_obj["id"])
        self.mock_brevo_client.add_contact_to_list.assert_called_once_with(email="stay@example.com", list_id=existing_list_obj["id"])
        self.assertEqual(len(results), 2)
        ...
        self.assertEqual(removed_action["mm_user_email"], "remove@example.com"); self.assertEqual(ensured_action["mm_user_email"], "stay@example.com")

    @patch("libraries.group_sync_services.config")
    def test_sync_brevo_list_excluded_user_not_added_or_removed(self, mock_lib_config_brevo):
        excluded_username = "excluded_brevo_user"; mock_lib_config_brevo.EXCLUDED_USERS = {excluded_username}
        brevo_list_name = "TestBrevoListExcluded"; existing_list_obj = {"id": "brevo_list_id_789", "name": brevo_list_name}
        self.mock_brevo_client.get_list_by_name.return_value = existing_list_obj
        mm_users_in_channel = [{"username": excluded_username, "email": "excluded_brevo@example.com"}, {"username": "normal_user", "email": "normal@example.com"}]
        self.mock_brevo_client.reset_mock()
        self.mock_brevo_client.get_list_by_name.return_value = existing_list_obj
        self.mock_brevo_client.get_contacts_from_list.return_value = [{"email": "unmanaged@example.com"}]
        results_rerun = self.sync_single_brevo_list_helper(self.mock_brevo_client, brevo_list_name, mm_users_in_channel, "MMChannelForBrevoExcluded", perform_deletions=True)
        self.mock_brevo_client.add_contact_to_list.assert_called_once_with(email="normal@example.com", list_id=existing_list_obj["id"])
        self.mock_brevo_client.remove_contact_from_list.assert_called_once_with(email="unmanaged@example.com", list_id=existing_list_obj["id"])
        actions_for_excluded = [r for r in results_rerun if r.get("mm_user_email") == "excluded_brevo@example.com"]
        self.assertEqual(len(actions_for_excluded), 0, "No direct add/remove actions should be logged for excluded user based on MM channel presence.")

    def sync_single_brevo_list_helper(self, mock_brevo_client, brevo_list_name, mm_users, mm_channel_name_log, perform_deletions):
        from libraries.group_sync_services import _sync_single_brevo_list as actual_sync_function
        return actual_sync_function(
            brevo_client=mock_brevo_client, brevo_list_name=brevo_list_name, mm_users_in_channel=mm_users,
            mm_channel_display_name_for_log=mm_channel_name_log, perform_deletions=perform_deletions,
        )
