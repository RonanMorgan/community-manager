...
class TestGroupSyncServices(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_vaultwarden_client = MagicMock(
            spec=VaultwardenClient
        )  # Added Vaultwarden mock
        self.mock_vaultwarden_client.organization_id = (
            "test_vw_org_id"  # Mock organization_id
        )
        ...
        self.mock_authentik_client.create_group.side_effect = (
            create_auth_group_side_effect
        )

        def create_outline_coll_side_effect(name):
            return {"name": name, "id": f"new_outline_id_for_{slugify(name)}"}

        self.mock_outline_client.create_group.side_effect = (
            create_outline_coll_side_effect
        )
    ...
    def test_extract_base_name(self):
        ...
        self.assertEqual(
            _extract_base_name("projet_TestProjet", "projet_{base_name}"), "TestProjet"
        )
        self.assertEqual(
            _extract_base_name("TestProjet_dev", "{base_name}_dev"), "TestProjet"
        )
        self.assertIsNone(
            _extract_base_name("projet_TestProjet_dev", "antenne_{base_name}_dev")
        )
        self.assertIsNone(
            _extract_base_name(
                "projet_TestProjet", "projet_{base_name}_suffix_mismatch"
            )
        )
        self.assertIsNone(
            _extract_base_name("projet_TestProjet", "exact_name_no_placeholder")
        )
        self.assertEqual(
            _extract_base_name("Projet Alpha", "Projet {base_name}"), "Alpha"
        )
        self.assertEqual(
            _extract_base_name("Projet Super Cool", "Projet {base_name}"), "Super Cool"
        )
        self.assertIsNone(
            _extract_base_name("Projet Admin", "Projet {base_name} Admin")
        )
        self.assertEqual(
            _extract_base_name("ProjetAdmin", "Projet{base_name}Admin"), ""
        )  # No spaces around base_name
        self.assertEqual(
    ...
    def test_map_auth_group_to_entity_and_base_name(self):
        matrix = {
            ...
            "ANTENNE": {
                "standard": {
                    "authentik_group_name_pattern": "antenne_{base_name}_standard"
                },
            },
        }
        self.assertEqual(
        ...
        self.assertEqual(
            _map_auth_group_to_entity_and_base_name(
                "antenne_MaRegion_standard", matrix
            ),
            ("ANTENNE", "MaRegion"),
        )
        self.assertIsNone(
            _map_auth_group_to_entity_and_base_name("unknown_group_format", matrix)[0]
        )
    ...
    def test_map_mm_channel_to_entity_and_base_name(self):
        matrix = {
            ...
            "ANTENNE": {
                "standard": {
                    "mattermost_channel_name_pattern": "Antenne {base_name} Standard"
                },
            },
        }
        self.assertEqual(
            _map_mm_channel_to_entity_and_base_name(
                "projet-alpha", "Projet Alpha", matrix
            ),
            ("PROJET", "Alpha"),
        )
        self.assertEqual(
            _map_mm_channel_to_entity_and_base_name(
                "projet-alpha-admin", "Projet Alpha Admin", matrix
            ),
            ("PROJET", "Alpha"),
        )
        self.assertEqual(
            _map_mm_channel_to_entity_and_base_name(
                "antenne-maregion-standard", "Antenne MaRegion Standard", matrix
            ),
            ("ANTENNE", "MaRegion"),
        )
        self.assertIsNone(
            _map_mm_channel_to_entity_and_base_name(
                "unknown-channel", "Unknown Channel Format", matrix
            )[0]
        )
    ...
    def test_sync_single_group_user_exclusion(self, mock_config_module_in_service):
        ...
        std_auth_group_name = mock_entity_config["standard"][
            "authentik_group_name_pattern"
        ].format(base_name=base_name)
        std_mm_channel_name = mock_entity_config["standard"][
            "mattermost_channel_name_pattern"
        ].format(base_name=base_name)
        adm_auth_group_name = mock_entity_config["admin"][
            "authentik_group_name_pattern"
        ].format(base_name=base_name)
        adm_mm_channel_name = mock_entity_config["admin"][
            "mattermost_channel_name_pattern"
        ].format(base_name=base_name)
        outline_coll_name = mock_entity_config["outline"][
        ...
        self.mock_mattermost_client.get_channel_by_name.side_effect = (
            get_channel_by_name_side_effect
        )
        ...
        self.mock_mattermost_client.get_users_in_channel.side_effect = (
            get_users_in_channel_side_effect
        )
        ...
        self.mock_authentik_client.add_user_to_group.assert_any_call(
            std_auth_group_obj["pk"], user1_pk
        )
        self.mock_authentik_client.add_user_to_group.assert_any_call(
            adm_auth_group_obj["pk"], user1_pk
        )
        self.mock_authentik_client.add_user_to_group.assert_any_call(
            adm_auth_group_obj["pk"], user2_pk
        )
    ...
    @patch("builtins.open")
    @patch("os.path.exists")
    def test_config_loading_empty_file(
        self, mock_exists, mock_open_file, mock_getenv, mock_find_dotenv
    ):
    ...
    @patch("builtins.open")
    @patch("os.path.exists")
    def test_config_loading_excluded_users_success(
        self, mock_exists, mock_open_file, mock_getenv, mock_find_dotenv
    ):
...
"""
    ...
    def test_sync_single_group_outline_dm_on_new_add(
        self, mock_config_module_in_service
    ):
        ...
        outline_success_results = [
            r for r in successful_sync_actions if r["service"] == "OUTLINE"
        ]
        self.assertEqual(
    ...
    @patch("libraries.group_sync_services.config")
    def test_sync_single_group_outline_user_already_member_no_dm(
        self, mock_config_module_in_service
    ):
        mock_config_module_in_service.EXCLUDED_USERS = set()
        mock_config_module_in_service.OUTLINE_URL = "http://fake-outline.com"
    ...
    def test_sync_outline_dm_skipped_no_outline_url(
        ...
        self.mock_authentik_client.add_user_to_group.return_value = (
            True  # Auth part succeeds
        )
        self.mock_outline_client.get_user_by_email.return_value = {
            "id": "outline_id_no_url"
        }
        expected_collection_id = f"new_outline_id_for_{slugify(outline_coll_name)}"
        self.mock_outline_client.create_group.return_value = {
            "id": expected_collection_id,
        ...
        self.mock_outline_client.add_user_to_collection.return_value = (
            True  # Outline add succeeds
        )
    ...
    def test_sync_outline_dm_skipped_incomplete_details(
        ...
        mock_config_module_in_service.EXCLUDED_USERS = set()
        mock_config_module_in_service.OUTLINE_URL = (
            "http://test-outline.com"  # URL is set
        )
    ...
    @patch("libraries.group_sync_services.config")
    def test_sync_single_group_authentik_user_removed_if_not_in_mm(
        self, mock_config_module_in_service
    ):
        mock_config_module_in_service.EXCLUDED_USERS = set()
    ...
    def test_sync_single_group_authentik_excluded_user_not_removed_if_not_in_mm(
        ...
        all_auth_groups_fixture = {
            current_auth_group_name: authentik_group_with_excluded_user
        }
        ...
        self.mock_authentik_client.add_user_to_group.assert_not_called()
        action_for_excluded_user_found = any(
            r["mm_username"] == excluded_auth_username for r in results
        )
        self.assertFalse(
    ...
    @patch("libraries.group_sync_services.config")
    def test_sync_single_group_outline_user_removed_if_not_in_mm(
        self, mock_config_module_in_service
    ):
        mock_config_module_in_service.EXCLUDED_USERS = set()
        mock_config_module_in_service.OUTLINE_URL = "http://fake-outline.com"
        ...
        self.mock_outline_client.get_user_by_email.side_effect = (
            mock_get_user_by_email_side_effect
        )
        ...
        removal_action = next(
            (
                r
                for r in results
                if r.get("action") == "USER_REMOVED_FROM_OUTLINE_COLLECTION"
            ),
            None,
        )
    ...
    def test_sync_single_group_outline_excluded_user_not_removed(
        ...
        all_auth_groups_fixture = {std_auth_group_name: auth_group}
        email_to_pk_map = {
            "excluded.outline@example.com": "auth_pk_excl_for_outline_test"
        }
        std_mm_channel_name = f"projet_{base_name_for_test}"
        std_mm_channel_obj = {
        ...
        action_for_excluded_user_in_outline = any(
            r["service"] == "OUTLINE" and r.get("mm_username") == excluded_mm_username
            for r in results
        )
        self.assertFalse(
    ...
    def test_sync_single_group_outline_permissions(self, mock_config_module_in_service):
        ...
        for (
            ...
            with self.subTest(subTest_name):
                ...
                self.mock_outline_client.create_group.side_effect = (
                    default_outline_create_group_side_effect
                )
                ...
                std_auth_group_name = mock_entity_config["standard"][
                    "authentik_group_name_pattern"
                ].format(base_name=base_name_from_case)
                std_mm_channel_name = mock_entity_config["standard"][
                    "mattermost_channel_name_pattern"
                ].format(base_name=base_name_from_case)
                adm_auth_group_name = mock_entity_config["admin"][
                ...
                expected_outline_coll_id = (
                    f"new_outline_id_for_{slugify(outline_coll_name)}"
                )
                ...
                self.mock_outline_client.create_group.assert_called_once_with(
                    outline_coll_name
                )
                self.mock_outline_client.add_user_to_collection.assert_called_once()
                called_args_add_user = (
                    self.mock_outline_client.add_user_to_collection.call_args
                )
                ...
                self.assertEqual(
                    called_args_add_user.kwargs.get("permission"), expected_permission
                )
                ...
                outline_result = next(
                    (
                        r
                        for r in results
                        if r["service"] == "OUTLINE" and r["status"] == "SUCCESS"
                    ),
                    None,
                )
                self.assertIsNotNone(
    ...
    async def test_orchestrate_sync_fetch_remote_false_discover_via_mm_no_deletions(
        ...
        mock_get_all_auth_groups_and_map.assert_called_once_with(
        ...
        self.mock_mattermost_client.get_channels_for_team.assert_called_once_with(
            mock_team_id
        )
    ...
    async def test_orchestrate_sync_fetch_remote_true_discover_via_auth_with_deletions(
        ...
        mock_sync_entity_permissions_call.return_value = [
            {"status": "SUCCESS", "action": "MOCKED_FULL_SYNC"}
        ]
        ...
        expected_all_auth_groups_by_name = {
            g["name"]: g for g in mock_all_auth_groups_list
        }
    ...
    def test_sync_brevo_list_user_removal(self, mock_lib_config_brevo):
        ...
        removed_action = next(
            r for r in results if r["action"] == "USER_REMOVED_FROM_BREVO_LIST"
        )
        ensured_action = next(
            r for r in results if r["action"] == "USER_ENSURED_IN_BREVO_LIST"
        )
    ...
    def test_sync_brevo_list_excluded_user_not_added_or_removed(
        ...
        actions_for_excluded = [
            r for r in results if r.get("mm_user_email") == "excluded_brevo@example.com"
        ]
        self.assertEqual(
    ...
    @patch(
        "libraries.group_sync_services.config"
    )  # To mock EXCLUDED_USERS and NOCODB_URL
    def test_sync_nocodb_base_creation_and_user_invite_with_dm(
        self, mock_lib_config_nocodb
    ):
        mock_lib_config_nocodb.EXCLUDED_USERS = set()
        mock_lib_config_nocodb.NOCODB_URL = (
            "https://test-nocodb.example.com"  # Mock NOCODB_URL for DM link
        )
        ...
        self.mock_nocodb_client.invite_user_to_base.return_value = True
        self.mock_mattermost_client.send_dm.return_value = (
            True  # Assume DMs are sent successfully
        )
        ...
        self.mock_nocodb_client.get_base_by_title.assert_called_once_with(
            nocodb_base_title
        )
        self.mock_nocodb_client.list_base_users.assert_called_once_with(
            "nc_base_id_123"
        )
        ...
        self.mock_nocodb_client.invite_user_to_base.assert_any_call(
            "nc_base_id_123", "user1@nocodb.com", default_perm
        )
        self.mock_nocodb_client.invite_user_to_base.assert_any_call(
            "nc_base_id_123", "admin@nocodb.com", admin_perm
        )
        ...
        self.assertTrue(
            dm_call_admin1_found, "DM call for admin1 (mm_nc_a1) not found."
        )
    ...
    def test_sync_nocodb_base_invite_dm_fails(self, mock_lib_config_nocodb):
        ...
        )  # Invite itself was successful
        self.assertEqual(
            results[0]["action"], "NOCODB_USER_INVITED_AS_VIEWER_DM_FAILED"
        )
        self.mock_mattermost_client.send_dm.assert_called_once()
    ...
    def test_sync_nocodb_base_invite_dm_skipped_no_url(self, mock_lib_config_nocodb):
        ...
        self.assertEqual(
            results[0]["action"], "NOCODB_USER_INVITED_AS_VIEWER_DM_SKIPPED_NO_URL"
        )
        self.mock_mattermost_client.send_dm.assert_not_called()
    ...
    def test_sync_nocodb_base_user_update_and_removal(self, mock_lib_config_nocodb):
        mock_lib_config_nocodb.EXCLUDED_USERS = set()
        mock_lib_config_nocodb.NOCODB_URL = "https://test-nocodb.example.com"  # For consistency, though not strictly needed for removal/update tests
        from libraries.group_sync_services import _sync_single_nocodb_base
        ...
        self.mock_nocodb_client.update_base_user.return_value = True
        self.mock_nocodb_client.delete_base_user.return_value = (
            True  # This actually sets role to no-access
        )
        self.mock_nocodb_client.invite_user_to_base.return_value = (
            True  # For user2 who is new
        )
        ...
        self.mock_nocodb_client.update_base_user.assert_any_call(
            base_id, "nc_uid1", "viewer"
        )
        # Check invite for user2
        self.mock_nocodb_client.invite_user_to_base.assert_any_call(
            base_id, "user2.owner@nocodb.com", "owner"
        )
        # Check removal for user.remove@nocodb.com
        self.mock_nocodb_client.delete_base_user.assert_called_once_with(
            base_id, "nc_uid_remove"
        )
    ...
    def test_sync_nocodb_base_excluded_user_handling(self, mock_lib_config_nocodb):
        ...
        self.mock_nocodb_client.delete_base_user.assert_any_call(
            base_id, "nc_uid_remove_excl_test"
        )

        actions = {
            r["mm_user_email"]: r["action"] for r in results if "mm_user_email" in r
        }
        ...
        self.assertEqual(
            actions.get("remove.excl@nocodb.com"), "NOCODB_USER_REMOVED_FROM_BASE"
        )
        self.assertNotIn(
            "excluded.user@nocodb.com", actions
        )  # No action logged for excluded user if already present
    ...
    def test_sync_entity_permissions_skip_nocodb(self, mock_lib_config):
        ...
        self.mock_brevo_client.get_lists.return_value = [
            {"id": "brevo_list_skip_id", "name": "brevo_list_skip"}
        ]
    ...
    def test_sync_vaultwarden_collection_invite_with_dm(self, mock_lib_config_vw):
        ...
        self.mock_vaultwarden_client.get_collection_by_name.return_value = (
            "vw_coll_id_123"
        )
        ...
        self.mock_vaultwarden_client.get_collection_by_name.assert_called_once_with(
            collection_name
        )
        ...
        self.assertEqual(
            results[0]["action"], "USER_INVITED_TO_VW_COLLECTION_AND_DM_SENT"
        )
    ...
    def test_sync_vaultwarden_invite_dm_fails(self, mock_lib_config_vw):
        ...
        self.assertEqual(results[0]["status"], "SUCCESS")
        self.assertEqual(
            results[0]["action"], "USER_INVITED_TO_VW_COLLECTION_DM_FAILED"
        )
    ...
    def test_sync_vaultwarden_invite_dm_skipped_no_url(self, mock_lib_config_vw):
        ...
        self.assertEqual(results[0]["status"], "SUCCESS")
        self.assertEqual(
            results[0]["action"], "USER_INVITED_TO_VW_COLLECTION_DM_SKIPPED_NO_URL"
        )
        self.mock_mattermost_client.send_dm.assert_not_called()
    ...
    def test_sync_vaultwarden_invite_fails_no_dm(self, mock_lib_config_vw):
        ...
        self.mock_vaultwarden_client.get_collection_by_name.return_value = (
            "vw_coll_id_invite_fail"
        )
        self.mock_vaultwarden_client._get_api_token.return_value = "fake_vw_api_token"
        self.mock_vaultwarden_client.invite_user_to_collection.return_value = (
            False  # Simulate invite failure
        )
