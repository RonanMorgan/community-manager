import unittest
from unittest.mock import patch, MagicMock, call  # Removed mock_open
import subprocess
import json
import os
...
class TestVaultwardenClient(unittest.TestCase):
    ...
    def test_configure_server_already_correct(self, mock_subprocess_run):
        mock_subprocess_run.return_value = subprocess.CompletedProcess(
            args=["bw", "status"],
            returncode=0,
            stdout=json.dumps({"serverUrl": self.server_url, "status": "unlocked"}),
            stderr="",
        )
        client = VaultwardenClient(organization_id=self.organization_id, server_url=self.server_url)
        self.assertIsNotNone(client)  # Use client to satisfy F841
    ...
    def test_configure_server_needs_update(self, mock_subprocess_run):
        ...
        mock_subprocess_run.side_effect = [
            subprocess.CompletedProcess(
                args=["bw", "status"],
                returncode=0,
                stdout=json.dumps({"serverUrl": "https://old.server.com", "status": "unlocked"}),
                stderr="",
            ),
            subprocess.CompletedProcess(args=["bw", "logout"], returncode=0, stdout="", stderr=""),
            subprocess.CompletedProcess(
                args=["bw", "config", "server", self.server_url], returncode=0, stdout="", stderr=""
            ),
        ]
    ...
    def test_get_session_valid_existing_session(self, mock_subprocess_run):
        ...
        mock_subprocess_run.side_effect = [
            ...
            subprocess.CompletedProcess(
                args=["bw", "unlock", "--check", "--session", "EXISTING_SESSION_TOKEN"],
                returncode=0,
                stdout="Success",
                stderr="",
            ),
            subprocess.CompletedProcess(
                args=["bw", "sync", "--session", "EXISTING_SESSION_TOKEN"], returncode=0, stdout="Synced", stderr=""
            ),
        ]
        client = VaultwardenClient(
            organization_id=self.organization_id, server_url=self.server_url, session_token="EXISTING_SESSION_TOKEN"
        )
    ...
    def test_get_session_unlock_with_password(self, mock_subprocess_run, mock_temp_file):
        ...
        mock_subprocess_run.side_effect = [
            subprocess.CompletedProcess(
                args=["bw", "status"],
                returncode=0,
                stdout=json.dumps({"serverUrl": self.server_url, "status": "locked"}),
                stderr="",
            ),
            subprocess.CompletedProcess(
                args=["bw", "unlock", "--raw"], returncode=0, stdout="NEWSESSIONTOKEN", stderr=""
            ),  # No newline
            subprocess.CompletedProcess(
                args=["bw", "sync", "--session", "NEWSESSIONTOKEN"], returncode=0, stdout="Synced", stderr=""
            ),
        ]

        client = VaultwardenClient(
            organization_id=self.organization_id, server_url=self.server_url, session_token=None
        )
        session = client._get_session()
    ...
    def test_get_session_locked_no_password_raises_error(self, mock_subprocess_run):
        ...
        if "BW_PASSWORD" in os.environ:
            del os.environ["BW_PASSWORD"]  # Ensure no password
    ...
    def test_create_collection_success(self, mock_subprocess_run, mock_get_session):
        ...
        mock_encode_call = subprocess.CompletedProcess(
            args=["bw", "encode"], returncode=0, stdout="ENCODED_DATA", stderr=""
        )
        created_coll_id = "coll-id-123"
        mock_create_call = subprocess.CompletedProcess(
            args=["bw", "create", "org-collection"],
            returncode=0,
            stdout=json.dumps(
                {"id": created_coll_id, "name": "Test Collection", "organizationId": self.organization_id}
            ),
            stderr="",
        )
    ...
    def test_create_collection_cli_create_fails(self, mock_subprocess_run, mock_get_session):
        ...
        mock_subprocess_run.side_effect = [
            ...
            subprocess.CompletedProcess(
                args=["bw", "create", "org-collection"], returncode=1, stdout="", stderr="Create error"
            ),
        ]
        client = VaultwardenClient(organization_id=self.organization_id, server_url=self.server_url)
        result = client.create_collection("Test Collection")
    ...
    def test_create_collection_non_json_output(self, mock_subprocess_run, mock_get_session):
        ...
        mock_subprocess_run.side_effect = [
            ...
            subprocess.CompletedProcess(args=["bw", "encode"], returncode=0, stdout="ENCODED_DATA", stderr=""),
            subprocess.CompletedProcess(
                args=["bw", "create", "org-collection"], returncode=0, stdout=raw_output_str, stderr=""
            ),
        ]
        client = VaultwardenClient(organization_id=self.organization_id, server_url=self.server_url)
        result = client.create_collection("Test Collection")
