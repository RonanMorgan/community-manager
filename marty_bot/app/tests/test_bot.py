...
class TestMartyBot(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_config.VAULTWARDEN_CLIENT_ID = (
            "fake_vw_client_id"  # Added for new client init
        )
        self.mock_config.VAULTWARDEN_CLIENT_SECRET = (
            "fake_vw_client_secret"  # Added for new client init
        )
        ...
        self.mock_config.PERMISSIONS_MATRIX = {
            ...
            "ANTENNE": {
                ...
                "vaultwarden": {
                    "collection_name_pattern": "VW_Antenne_{base_name}"
                },  # Added
            },
            "POLES": {
                "standard": {
                ...
                "vaultwarden": {
                    "collection_name_pattern": "VW_Pole_{base_name}"
                },  # Added
            },
        }
    ...
    async def _send_test_message(
        self, message_text, channel_id="test_channel", user_id=None
    ):
    ...
    async def test_handle_create_projet_command_single_item_success_and_user_added(
        ...
        self.bot.brevo_client.get_list_by_name.return_value = (
            None  # Simulate list does not exist initially
        )
        ...
        self.bot.mattermost_api_client.create_channel.side_effect = (
            create_channel_side_effect
        )
        self.bot.mattermost_api_client.add_user_to_channel.return_value = True
        await self._send_test_message(
            f"@{self.mock_config.BOT_NAME} create_projet {project_name}"
        )
        ...
        self.bot.outline_client.create_group.assert_called_once_with(
            expected_outline_coll_name
        )
        self.bot.brevo_client.get_folder_id_by_name.assert_called_once_with(
            "Dossier Projets Test"
        )
        self.bot.brevo_client.get_list_by_name.assert_called_once_with(
            expected_brevo_list_name
        )
        self.bot.brevo_client.create_list.assert_called_once_with(
            expected_brevo_list_name, folder_id=mocked_folder_id
        )
        self.bot.mattermost_api_client.create_channel.assert_any_call(
            expected_std_mm_name, channel_type="O"
        )
        self.bot.mattermost_api_client.create_channel.assert_any_call(
            expected_adm_mm_name, channel_type="P"
        )
        ...
        self.assertEqual(
            self.bot.mattermost_api_client.add_user_to_channel.call_count, 2
        )
    ...
    async def test_handle_create_projet_command_multiple_items_success(self):
        ...
        self.bot.brevo_client.get_folder_id_by_name.return_value = (
            123  # Mock folder ID for these tests
        )
        ...
        self.bot.mattermost_api_client.add_user_to_channel.return_value = True
        await self._send_test_message(
            f"@{self.mock_config.BOT_NAME} create_projet {' '.join(project_names_input)}"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.bot.outline_client.create_group.call_count, len(project_names_input)
        )
        self.assertEqual(
            self.bot.brevo_client.create_list.call_count, len(project_names_input)
        )
        self.assertEqual(
    ...
    async def test_handle_create_antenne_command_multiple_items(self):
        ...
        self.bot.nocodb_client.get_base_by_title.return_value = (
            None  # Simulate base does not exist
        )
        ...
        await self._send_test_message(
            f"@{self.mock_config.BOT_NAME} create_antenne {' '.join(antenne_names_input)}"
        )
        ...
        self.assertEqual(
            self.bot.brevo_client.create_list.call_count, len(antenne_names_input)
        )
        self.assertEqual(
            self.bot.nocodb_client.create_base.call_count, len(antenne_names_input)
        )  # Check NocoDB calls
        self.assertEqual(self.bot.envoyer_message.call_count, 2)
    ...
    async def test_handle_create_pole_command_multiple_items(self):
        ...
        self.mock_config.PERMISSIONS_MATRIX["POLES"]["brevo"][
            "folder_name"
        ] = "Dossier Poles Test"
        ...
        await self._send_test_message(
            f"@{self.mock_config.BOT_NAME} create_pole {' '.join(pole_names_input)}"
        )

        self.assertEqual(
            self.bot.authentik_client.create_group.call_count, len(pole_names_input) * 2
        )
        self.assertEqual(
            self.bot.brevo_client.create_list.call_count, len(pole_names_input)
        )
        self.assertEqual(
            self.bot.nocodb_client.create_base.call_count, len(pole_names_input)
        )  # Check NocoDB
        self.assertEqual(self.bot.envoyer_message.call_count, 2)
    ...
    async def test_create_commands_no_arg_provided(self):
        ...
        for cmd, item_type in commands_to_test.items():
            ...
            self.assertIn(
                f":warning: Au moins un nom de {item_type} est requis.", sent_message
            )
    ...
    async def test_create_resources_for_category_client_errors(self):
        ...
        self.bot.nocodb_client.get_base_by_title.return_value = (
            None  # NocoDB base does not exist
        )
        ...
        with patch.object(
            ...
            await self._send_test_message(
                f"@{self.mock_config.BOT_NAME} create_projet {project_name_input}"
            )
            summary_text_projet = self.bot.envoyer_message.call_args_list[1][0][
                1
            ]  # Second call is summary
            mock_get_id_method_projet.assert_called_once_with(expected_folder_name)
            self.assertNotIn(
                "NoCoDB Base", summary_text_projet
            )  # NocoDB should not be mentioned for PROJET
        ...
        antenne_expected_folder_name = antenne_brevo_config.get(
            "folder_name"
        )  # Might be None if not set for Antenne
        ...
        with patch.object(
            ...
            await self._send_test_message(
                f"@{self.mock_config.BOT_NAME} create_antenne {antenne_name_input}"
            )
            summary_text_antenne = self.bot.envoyer_message.call_args_list[1][0][1]
            if antenne_expected_folder_name:
                mock_get_id_method_antenne.assert_called_once_with(
                    antenne_expected_folder_name
                )
    ...
    async def test_handle_simple_mention_unknown_command(self):
        await self._send_test_message(
            f"@{self.mock_config.BOT_NAME} hello there", channel_id="general"
        )
    ...
    async def test_handle_mention_no_command(self):
        await self._send_test_message(
            f"@{self.mock_config.BOT_NAME}", channel_id="town-square"
        )
        self.bot.envoyer_message.assert_called_once_with(
            "town-square",
    ...
    def test_parse_command_from_mention_logic(self):
        ...
        self.assertEqual(
            self.bot._parse_command_from_mention("help   "), ("help", None)
        )
        self.assertEqual(
    ...
    def test_handle_update_all_user_rights_command_success(self, mock_orchestrate_sync):
        async def actual_test_logic():
            ...
            await self._send_test_message(
                f"@{self.mock_config.BOT_NAME} {command_name}", user_id=admin_user_id
            )

            self.bot.mattermost_api_client.get_user_roles.assert_called_once_with(
                admin_user_id
            )
    ...
    @patch("app.bot.orchestrate_group_synchronization")
    def test_handle_update_user_rights_and_remove_command_success_admin_user(
        self, mock_orchestrate_sync
    ):
        async def actual_test_logic():
            ...
            for call_args_tuple in self.bot.envoyer_message.call_args_list:
                ...
                if "Résumé de Suppression/synchronisation des droits" in message_text:
                    ...
                    break
            self.assertTrue(
                summary_call_found, "Summary message for full sync/remove not found."
            )
    ...
    @async_test
    @patch("app.bot.orchestrate_group_synchronization")
    async def test_sync_commands_permission_denied_non_admin(
        self, mock_orchestrate_sync
    ):
        commands_to_test = [
    ...
    async def test_sync_commands_orchestration_failure_admin_user(self):
        ...
        with patch.object(
            self.bot, "orchestrate_group_synchronization", return_value=(False, [])
        ) as mock_orchestrate:
            for command_key in commands_to_test:
                with self.subTest(command=command_key):
                    self.bot.envoyer_message.reset_mock()
                    ...
                    self.assertEqual(self.bot.envoyer_message.call_count, 2)
                    final_message_text = self.bot.envoyer_message.call_args_list[1][0][
                        1
                    ]
                    self.assertIn(
    ...
    async def test_sync_commands_no_clients_configured_admin_user(self):
        ...
        for command_key in commands_to_test:
            with self.subTest(command=command_key):
                ...
                await self._send_test_message(
                    f"@{self.mock_config.BOT_NAME} {command_key}", user_id=admin_user_id
                )
                ...
                self.assertIn(
                    "Le bot n'est pas correctement configuré", error_message_text
                )
    ...
    async def test_handle_create_projet_calls_vaultwarden_client(self):
        ...
        self.bot.vaultwarden_client.create_collection.assert_called_once_with(
            expected_vw_collection_name
        )
    ...
    @async_test
    @patch("app.bot.orchestrate_group_synchronization")
    async def test_handle_update_user_rights_and_remove_command_with_skip_nocodb(
        self, mock_orchestrate_sync
    ):
        command_name = "update_user_rights_and_remove"
...
class TestSendEmailCommand(TestMartyBot):
    ...
    def test_handle_send_email_success(self, mock_map_channel, mock_slugify_call):
        async def actual_test_logic():
            ...
            admin_channel_config = self.mock_config.PERMISSIONS_MATRIX[
                entity_key_for_test
            ]["admin"]
            admin_channel_display_name = admin_channel_config[
                "mattermost_channel_name_pattern"
            ].format(base_name=base_name_for_test)
            admin_channel_slug = slugify(admin_channel_display_name)

            def map_channel_side_effect(
                ch_slug_arg, ch_display_name_arg, entity_config_slice_arg
            ):
            ...
            brevo_list_name_pattern_from_config = self.mock_config.PERMISSIONS_MATRIX[
                entity_key_for_test
            ]["brevo"]["list_name_pattern"]
            expected_brevo_list_name = brevo_list_name_pattern_from_config.format(
            ...
            projet_config_slice = {
                "PROJET": self.mock_config.PERMISSIONS_MATRIX["PROJET"]
            }
            mock_map_channel.assert_any_call(
                admin_channel_slug, admin_channel_display_name, projet_config_slice
            )

            self.bot.brevo_client.get_list_by_name.assert_called_once_with(
                expected_brevo_list_name
            )
            self.bot.brevo_client.get_contacts_from_list.assert_called_once_with(
                "brevo_list_123"
            )
    ...
    def test_handle_send_email_brevo_list_not_found(self, mock_map_channel):
        async def actual_test_logic():
            ...
            entity_key_for_test = "PROJET"
            admin_display_name = self.mock_config.PERMISSIONS_MATRIX[
                entity_key_for_test
            ]["admin"]["mattermost_channel_name_pattern"].format(
                base_name=base_name_for_test
            )
            ...
            def map_channel_side_effect(
                ...
                if (
                    iter_entity_key == entity_key_for_test
                    and ch_display_name_arg == admin_display_name
                ):
                    return (entity_key_for_test, base_name_for_test)
                return (None, None)
            ...
            self.bot.envoyer_message.assert_called_with(channel_id, unittest.mock.ANY)
            last_call_args = self.bot.envoyer_message.call_args[0]
            expected_brevo_list_name = self.mock_config.PERMISSIONS_MATRIX[
                entity_key_for_test
            ]["brevo"]["list_name_pattern"].format(base_name=base_name_for_test)
            self.assertIn(
    ...
    def test_handle_send_email_no_recipients_in_list(self, mock_map_channel):
        async def actual_test_logic():
            ...
            brevo_list_name_pattern = self.mock_config.PERMISSIONS_MATRIX[
                entity_key_for_test
            ]["brevo"]["list_name_pattern"]
            expected_brevo_list_name = brevo_list_name_pattern.format(
                base_name=base_name_for_test
            )
            self.bot.brevo_client.get_list_by_name.return_value = {
    ...
    def test_handle_send_email_bad_syntax(self):
        async def actual_test_logic():
            ...
            with patch(
                ...
                self.bot.envoyer_message.assert_called_with(
                    channel_id, unittest.mock.ANY
                )
                last_call_args = self.bot.envoyer_message.call_args[0]
