...
class HelpCommand(BaseCommand):
    ...
    async def execute(self, channel_id, arg_string, user_id_who_posted):
        ...
        if not self.bot.command_factory.commands:
            ...
            for cmd, handler_instance in sorted(
                self.bot.command_factory.commands.items()
            ):
                description = ""
        ...
        help_lines.append(
            f"* `{self.bot.bot_name_mention} create_antenne AntenneRegionale`"
        )
        help_lines.append(
            f"* `{self.bot.bot_name_mention} create_pole PoleTechnique AutrePole`"
        )
        help_lines.append(
        ...
        help_lines.append(
            f"* **`{self.bot.bot_name_mention} update_user_rights_and_remove`**"
        )
        help_lines.append(
        ...
        help_lines.append(
            f"* **`{self.bot.bot_name_mention} send_email <Sujet> /// <Message>`**"
        )
        help_lines.append(
