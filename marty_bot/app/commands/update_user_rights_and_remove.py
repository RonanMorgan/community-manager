...
class UpdateUserRightsAndRemoveCommand(BaseCommand):
    ...
    async def execute(self, channel_id, arg_string, user_id_who_posted):
        ...
        if not self.bot.mattermost_api_client or not user_id_who_posted:
            logging.error(
                "Mattermost API client or user_id_who_posted not available for permission check."
            )
        ...
        user_roles = await asyncio.to_thread(
            self.bot.mattermost_api_client.get_user_roles, user_id_who_posted
        )
        ...
        if arg_string and arg_string.lower() == "nocodb=false":
            ...
            logging.info(
                "NoCoDB synchronization will be skipped for this run based on 'nocodb=false' argument."
            )
            ...
            if (
                arg_string
            ):  # Log if there was an argument but it wasn't the recognized one
                logging.info(
        ...
        initial_post_id = await asyncio.to_thread(
            self.bot.envoyer_message, channel_id, initial_message_text
        )
        ...
        try:
            ...
            orchestration_success, detailed_results = (
                await self.bot.orchestrate_group_synchronization(
