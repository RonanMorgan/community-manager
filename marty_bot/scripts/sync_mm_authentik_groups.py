...
def main_sync_logic():
    ...
    if not authentik_client:
        logging.error(
            "Authentik client not initialized. Cannot proceed. " "Please check AUTHENTIK_URL and AUTHENTIK_TOKEN."
        )
        return
    if not mattermost_client:
        logging.error(
            "Mattermost client not initialized. Cannot proceed. "
            "Please check MATTERMOST_URL, BOT_TOKEN, and MATTERMOST_TEAM_ID."
        )
        return
