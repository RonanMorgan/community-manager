            ...
            if not is_already_member:
                ...
                if (
                    ...
                    if outline_base_url:
                        ...
                        logging.warning(
                            f"OUTLINE_URL not configured. Cannot send DM for Outline collection '{coll_name_for_dm}' "
                            f"to user '{mm_username}'."
                        )
                        current_action = f"{action_verb_prefix}_DM_SKIPPED_NO_URL"
                elif mm_user_data.get("mm_user_id"):
                    logging.warning(
                        f"Could not send DM for Outline collection (ID: {collection_id}) to user '{mm_username}' "
